...
import { Calendar, Clock, Video, Search, Filter, ArrowUpRight, User } from 'lucide-react';
    ...
    return (
        <div className="p-8 max-w-[1600px] mx-auto min-h-screen bg-gray-50/50">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div>
                    <h1 className="text-4xl font-black text-gray-900 tracking-tight">Team Meetings</h1>
                    <p className="text-gray-500 mt-2 font-bold text-lg">Scheduled syncs for your projects</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {!selectedProjectId ? (
                    <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-200">
                        ...
                        <h3 className="text-2xl font-black text-gray-400">Select a project</h3>
                        <p className="text-gray-400 font-bold mt-2">Please select a project to view its meetings</p>
                    </div>
                ) : isLoading ? (
                    [1, 2, 3].map(i => (
                    ...
                    meetings.map((meeting: any) => (
                        <div key={meeting.id} className="group bg-white p-8 rounded-[40px] shadow-sm hover:shadow-2xl hover:shadow-indigo-500/10 border border-gray-100 transition-all duration-500 hover:-translate-y-2 flex flex-col justify-between">
                            ...
                            <button
                                onClick={() => window.location.href = `/developers/meeting/${meeting.roomId}`}
                    ...
                    <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-gray-400 font-bold mt-2">No upcoming meetings for this project</p>
                    </div>
                )}
            </div>
        </div>
