...
function InfoRow({
	...
	return (
		<div className="flex flex-col gap-1">
			...
			<span
				className={`text-sm font-semibold ${value ? "text-slate-800" : "text-slate-300 italic"}`}
			>
				{value || placeholder || "—"}
			</span>
		</div>
...
export default function ProfilePage() {
	...
	const { data: profileResponse, isLoading: isFetching } = useGetProfile(
		user?.companyId,
	);
	...
	const onSubmit = (data: ProfileFormData) => {
		...
		updateProfile(
			{ companyId: user.companyId, payload },
			{
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-slate-500 mt-1">
						Manage your professional identity and workspace settings.
					</p>
				</div>
				{!isEditing ? (
					<button
