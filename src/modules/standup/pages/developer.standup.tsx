				...
				<div className="flex items-center gap-6 pl-2">
					...
					<div className="relative group ml-4">
						...
						>
							...
							<ChevronDown
								size={16}
								className={`text-gray-400 group-hover:text-indigo-500 transition-all duration-300 ${isDropdownOpen ? "rotate-180" : ""}`}
							/>
						</div>
									...
									{projects.map((p: any) => (
										...
										>
											<div className="flex items-center gap-3">
												<div
													className={`w-2 h-2 rounded-full transition-all ${selectedProjectId === (p.id || p._id) ? "bg-indigo-600 scale-125" : "bg-gray-200 group-hover/item:bg-indigo-300"}`}
												/>
												<span
													className={`text-sm font-bold ${selectedProjectId === (p.id || p._id) ? "text-indigo-600" : "text-gray-600"}`}
												>
													{p.name}
												</span>
											</div>
				...
				{selectedProjectId ? (
						...
						{showSprintOverview && (
							<aside className="w-96 border-l border-gray-100 bg-white overflow-y-auto scrollbar-none animate-in slide-in-from-right duration-500 hidden xl:block shadow-[-10px_0_30px_rgba(0,0,0,0.02)]">
								<SprintOverview
									...
									onSprintFound={(id) => {
										if (!urlSprintId && id)
											setParams({ projectId: selectedProjectId, sprintId: id });
									}}
								/>
							</aside>
						)}
					...
					<div className="flex-1 flex flex-col items-center justify-center p-20 text-center animate-in fade-in duration-1000">
						...
						<p className="text-gray-500 font-medium max-w-sm mx-auto text-lg leading-relaxed">
							Select a project from the switcher above to start your daily sync
							and view team progress.
						</p>
					</div>
				)}
			</main>
...
function SprintOverview({
	...
	}, [sprintId, onSprintFound]);

	const { data: storiesRes, isLoading: isLoadingStories } = useGetUserStories(
		projectId,
		{ sprintId },
	);
			...
			<div className="p-12 text-center space-y-6">
				...
				<div className="space-y-4">
					...
					<p className="text-gray-400 text-sm font-medium leading-relaxed px-4">
						...
						<br />
						<br />
						Wait for your project lead to start an active sprint cycle to post
						your daily updates.
					</p>
				</div>
			</div>
			...
			<div className="space-y-5">
				...
				<div>
					...
					<div className="flex items-center gap-4 mt-3">
						<div className="flex items-center gap-1.5 text-xs font-bold text-gray-400">
							...
							{new Date(runningSprint.startDate).toLocaleDateString(undefined, {
								month: "short",
								day: "numeric",
							})}{" "}
							-{" "}
							{new Date(runningSprint.endDate).toLocaleDateString(undefined, {
								month: "short",
								day: "numeric",
								year: "numeric",
							})}
				...
				<div className="bg-indigo-600 p-5 rounded-[2rem] border border-indigo-500 shadow-xl shadow-indigo-100 relative overflow-hidden group">
					...
					<p className="text-[13px] font-bold text-white leading-relaxed relative z-10 italic">
						"
						{runningSprint.goal ||
							"Focus on delivering core value and maintaining high code quality."}
						"
					</p>
				</div>
			</div>
				...
				<div className="space-y-4">
					{isLoadingStories ? (
						...
						stories.map((story: any) => (
							<div
								key={story._id}
								className="p-5 bg-white rounded-[1.5rem] border border-gray-100 shadow-sm hover:border-indigo-100 hover:shadow-md transition-all duration-300 group"
							>
								<div className="space-y-4">
