...
export function KanbanBoard({ tasks, onTaskMove, onSubtaskUpdate }: KanbanBoardProps) {
    ...
    const getTasksByStatus = (status: string) => {
        ...
        return tasks.filter((task) => {
            const taskStatus = (task.status || "pending").toLowerCase().replace(" ", "-");
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const currentStatus = (activeTask?.status || "").toLowerCase().replace(" ", "-");
        ...
        >
            <div className="flex h-full gap-6 overflow-x-auto pb-4">
                {COLUMNS.map((col) => {
                    ...
                    return (
                        <div key={col.id} className="flex-shrink-0 w-80 rounded-[32px] border border-white/[0.05] bg-white/[0.02] flex flex-col min-h-[500px]">
                            <div className="p-6 border-b border-white/[0.05] flex items-center gap-3">
                                <div className={`p-2 rounded-xl bg-${col.color}-500/10 border border-${col.color}-500/20`}>
                                    <Icon size={18} className={`text-${col.color}-400`} />
                                </div>
                                <div>
                                    <h3 className="font-black text-white text-lg">{col.title}</h3>
                                    <span className="text-xs text-gray-500 font-bold">{stories.length} tasks</span>
                                </div>
                            </div>
                                ...
                                <div className="flex-1 p-4 overflow-y-auto space-y-3">
                                    ...
                                    {stories.length === 0 && (
                                        <div className="py-10 text-center">
                                            <p className="text-xs font-bold text-gray-600">No tasks</p>
                                        </div>
                                    )}
                                </div>
                            </SortableContext>
