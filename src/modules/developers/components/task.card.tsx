    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-2">
                <div className="flex items-start gap-3 w-full">
                    ...
                    <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-sm text-white leading-snug mb-2 truncate">{task.title}</h4>
            ...
            <div className="mt-4 space-y-2">
                <p className="text-[10px] font-black text-gray-600 uppercase tracking-wider mb-2">Subtasks</p>
                {task.subtasks && task.subtasks.length > 0 ? (
                    task.subtasks.map((subtask) => {
                        const isAssignedToMe = subtask.assignedTo === myId;

                        return (
                            <div key={subtask._id} className="flex items-center justify-between bg-black/20 p-2 rounded-lg border border-white/5">
                                <span className="truncate flex-1 font-medium text-xs text-gray-300 mr-2">{subtask.title}</span>

                                {isAssignedToMe ? (
                                    <select
                                        value={subtask.status}
                                        onChange={(e) => onSubtaskUpdate(subtask._id, e.target.value)}
