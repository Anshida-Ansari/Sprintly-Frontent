...
export default function DashboardPage() {
	...
	useEffect(() => {
		...
		const handleMeetingScheduled = (data: any) => {
			toast.custom(
				(id) => (
					<div className="animate-in fade-in zoom-in duration-300 max-w-md w-full bg-white shadow-2xl rounded-[32px] pointer-events-auto flex ring-1 ring-black ring-opacity-5 p-4 border border-indigo-100">
						<div className="flex-1 w-0 p-4">
							<div className="flex items-start">
								...
								<div className="ml-4 flex-1">
									...
									<p className="mt-1 text-xs text-indigo-500 font-bold">
										{new Date(data.date).toLocaleString()}
									</p>
								</div>
							</div>
						</div>
						<div className="flex border-l border-gray-100 pl-4 items-center">
							<button
								onClick={() => {
									toast.dismiss(id);
									navigate(`/admin/meeting/${data.roomId}`);
								}}
								className="w-full border border-transparent rounded-none rounded-r-lg p-4 flex items-center justify-center text-sm font-black text-indigo-600 hover:text-indigo-500 focus:outline-none"
							>
								Join
							</button>
						</div>
		...
		<div className="max-w-6xl mx-auto space-y-12">
			...
			<header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
				<div>
					...
					<p className="text-gray-500 mt-2 font-mono text-sm uppercase tracking-widest">
						User: {user?.name || "User!"} // Rank: Senior_L4
					</p>
				</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<div className="lg:col-span-2 group bg-gradient-to-br from-[#16161A] to-[#0D0D0F] p-8 rounded-3xl border border-white/5 hover:border-indigo-500/30 transition-all duration-500 relative overflow-hidden">
					...
					<p className="text-indigo-500 text-xs font-black uppercase tracking-[0.3em] mb-4">
						Active Sprint
					</p>
					<h2 className="text-3xl text-white font-bold max-w-sm mb-6 leading-tight">
				...
				<div className="bg-[#111113] p-8 rounded-3xl border border-white/5 flex flex-col justify-between group">
					<div>
						...
						<p className="text-sm text-gray-500 mt-2 font-mono">
							fix(auth): solved token expiration race condition
						</p>
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				{[
					...
					<div
						key={i}
						className="bg-white/[0.02] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.04] transition-all"
					>
						...
						<div className="flex items-end justify-between mt-2">
							...
							<span
								className={`text-[10px] font-bold text-${item.color}-500 bg-${item.color}-500/10 px-2 py-0.5 rounded`}
							>
								{item.sub}
							</span>
						</div>
			...
			<div className="bg-[#0D0D0F] rounded-3xl border border-white/5 overflow-hidden">
				<div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
					<h3 className="text-white font-bold flex items-center gap-2">
						<CheckCircle2 size={18} className="text-indigo-500" /> Activity
						Stream
					</h3>
