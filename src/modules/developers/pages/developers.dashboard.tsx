						...
						<div className="flex-1 w-0 p-4">
							<div className="flex items-start">
								...
								<div className="ml-3 flex-1">
									...
									<p className="mt-1 text-sm text-gray-500 line-clamp-1">{data.title}</p>
		...
		return (
			<div className="flex items-center justify-center h-[60vh]">
				<div className="flex flex-col items-center gap-4">
					...
					<p className="text-gray-500 font-medium animate-pulse">Loading your briefing...</p>
				</div>
			</div>
		);
	}
				...
				<div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
					<div>
						...
						<p className="text-gray-500 text-lg">
							You have <span className="font-bold text-gray-900">{stats?.todayTasks?.length || 0}</span> tasks requiring attention today.
						</p>
					</div>

					<div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
						<div className="bg-slate-50 border border-gray-100 p-4 rounded-2xl text-center shadow-sm">
							<p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Weekly Tasks</p>
							<p className="text-2xl font-black text-emerald-600">{stats?.performance?.tasksCompletedThisWeek || 0}</p>
						</div>
						<div className="bg-slate-50 border border-gray-100 p-4 rounded-2xl text-center shadow-sm">
							<p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Hours Logged</p>
							<p className="text-2xl font-black text-indigo-600">{stats?.performance?.hoursWorkedThisWeek || 0}h</p>
						</div>
						<div className="hidden sm:block bg-slate-50 border border-gray-100 p-4 rounded-2xl text-center shadow-sm">
							...
							<p className="text-2xl font-black text-amber-600">{stats?.performance?.completionRate || 0}%</p>
						</div>
					</div>
				</div>
			</div>
					...
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							...
							{stats?.todayTasks?.length > 0 && (
								<span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">
									{stats.todayTasks.filter((t: any) => t.isOverdue).length} Overdue
								</span>
							)}
						</div>
						...
						{stats?.currentFocus ? (
							<div className="bg-gray-900 rounded-3xl p-8 text-white relative overflow-hidden group hover:scale-[1.01] transition-all cursor-pointer shadow-xl shadow-indigo-100">
								...
								<div className="relative z-10">
									<div className="flex items-center gap-2 mb-6">
										...
										<span className="text-gray-500 text-xs">— {stats.currentFocus.projectName}</span>
									</div>
									...
									<div className="flex flex-wrap items-center gap-6 mb-8">
										<div className="flex items-center gap-2">
											...
											<span className="text-sm font-medium text-gray-300">
												{stats.currentFocus.actualHours}h / {stats.currentFocus.estimatedHours}h
											</span>
										</div>
										<div className="flex items-center gap-2">
											<Calendar className="text-indigo-400" size={18} />
											<span className="text-sm font-medium text-gray-300">
												Due {stats.currentFocus.dueDate ? new Date(stats.currentFocus.dueDate).toLocaleDateString() : "No Date"}
											</span>
										</div>
									</div>
							...
							<div className="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center">
								...
								<h2 className="text-xl font-bold text-gray-900 mb-2">No active focus detected</h2>
								<p className="text-gray-500 max-w-xs mx-auto mb-6">
									Ready to dive in? Start by picking a task from your pending queue below.
								</p>
								...
								stats.todayTasks.map((task: any) => (
									<div key={task.id} className="p-5 flex items-center justify-between hover:bg-gray-50 transition-colors group">
										<div className="flex items-center gap-4">
											<div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${task.isOverdue ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400'}`}>
												{task.isOverdue ? <Clock size={20} /> : <FileText size={20} />}
											</div>
											<div>
												<p className="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-1">{task.title}</p>
												<div className="flex items-center gap-3 mt-1">
													<p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">{task.projectName}</p>
													<span className="w-1 h-1 bg-gray-300 rounded-full" />
													<p className={`text-[10px] font-bold uppercase ${task.isOverdue ? 'text-red-500' : 'text-gray-400'}`}>
														{task.isOverdue ? "Overdue" : `Due ${new Date(task.dueDate).toLocaleDateString()}`}
													</p>
												</div>
											</div>
										</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							...
							<div className="bg-slate-50/50 p-6 rounded-3xl border border-gray-100">
								<div className="flex items-center gap-2 mb-4">
									...
									<h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Pending</h4>
								...
								<div className="space-y-3">
									{stats?.myTasks?.pending?.slice(0, 3).map((t: any) => (
										<div key={t.id} className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm hover:border-indigo-200 transition-all cursor-pointer">
											<p className="text-xs font-bold text-gray-800 line-clamp-2">{t.title}</p>
											<p className="text-[10px] font-medium text-gray-400 mt-2">{t.projectName}</p>
										</div>
									))}
							...
							<div className="bg-indigo-50/30 p-6 rounded-3xl border border-indigo-100/50">
								<div className="flex items-center gap-2 mb-4">
									...
									<h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Active</h4>
								...
								<div className="space-y-3">
									{stats?.myTasks?.inProgress?.slice(0, 3).map((t: any) => (
										<div key={t.id} className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm hover:shadow-md transition-all cursor-pointer">
											<p className="text-xs font-bold text-gray-800 line-clamp-2">{t.title}</p>
											<div className="flex items-center gap-2 mt-2">
												<div className="flex-1 h-1.5 bg-gray-50 rounded-full overflow-hidden">
													<div 
														className="h-full bg-indigo-500 rounded-full" 
														style={{ width: `${(t.actualHours / t.estimatedHours) * 100 || 0}%` }}
													/>
												</div>
												<span className="text-[9px] font-bold text-indigo-600">{Math.round((t.actualHours / t.estimatedHours) * 100 || 0)}%</span>
											</div>
										</div>
									))}
								</div>
							...
							<div className="bg-emerald-50/30 p-6 rounded-3xl border border-emerald-100/50">
								<div className="flex items-center gap-2 mb-4">
									...
									<h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Done</h4>
								...
								<div className="space-y-3 opacity-80">
									{stats?.myTasks?.completed?.slice(0, 3).map((t: any) => (
										<div key={t.id} className="bg-white p-3 rounded-xl border border-emerald-100 shadow-sm flex items-center gap-2">
											<CheckCircle2 size={14} className="text-emerald-500 shrink-0" />
											<p className="text-xs font-bold text-gray-600 line-clamp-1 line-through">{t.title}</p>
										</div>
									))}
								</div>
							</div>
				...
				<div className="lg:col-span-4 space-y-8">
					...
					{stats?.activeSprint ? (
						<div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm relative overflow-hidden group">
							...
							<div className="relative z-10">
								<div className="flex items-center gap-2 mb-6">
									...
									<span className="text-gray-400 text-xs font-medium">{stats.activeSprint.daysLeft} days left</span>
								</div>
								
								<h3 className="text-2xl font-black text-gray-900 mb-6">{stats.activeSprint.name}</h3>
								
								<div className="space-y-6">
									<div>
										<div className="flex justify-between items-end mb-2">
											<p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Your Progress</p>
											<p className="text-xs font-black text-indigo-600">{stats.activeSprint.completionPercentage}%</p>
										</div>
										<div className="h-4 w-full bg-gray-50 rounded-full p-1 border border-gray-100 shadow-inner">
											<div 
												...
												style={{ width: `${stats.activeSprint.completionPercentage}%` }}
											/>
										</div>
										<p className="text-[10px] font-bold text-gray-400 mt-2 text-center uppercase tracking-widest">
											{stats.activeSprint.completedTasks} of {stats.activeSprint.totalTasks} tasks completed
										</p>
									</div>

									<div className="grid grid-cols-2 gap-4">
										<div className="bg-slate-50 p-3 rounded-2xl border border-gray-100 text-center">
											<p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Remaining</p>
											<p className="text-lg font-black text-gray-800">{stats.activeSprint.totalTasks - stats.activeSprint.completedTasks}</p>
										</div>
										<div className="bg-slate-50 p-3 rounded-2xl border border-gray-100 text-center">
											<p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Health</p>
						...
						<div className="bg-slate-50 p-8 rounded-3xl border border-gray-200 border-dashed text-center">
							<p className="text-sm font-bold text-gray-400">No active sprint</p>
						</div>
					)}
					...
					<div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm">
						<div className="flex items-center justify-between mb-6">
							...
							<button onClick={() => navigate("/developers/meetings")} className="text-[10px] font-bold text-indigo-600 hover:underline uppercase tracking-wider">
								View All
							</button>
						</div>
							...
							{stats?.schedule?.length > 0 ? (
								stats.schedule.map((meeting: any) => (
									<div key={meeting.id} className="relative pl-6 before:absolute before:left-0 before:top-2 before:bottom-0 before:w-0.5 before:bg-indigo-100">
										...
										<p className="text-sm font-bold text-gray-900 line-clamp-1">{meeting.title}</p>
										<p className="text-[10px] font-medium text-gray-400 mt-0.5">Google Meet • Sync</p>
									</div>
								))
							) : (
								<div className="text-center py-4">
									<p className="text-xs font-medium text-gray-400 italic">No meetings today.</p>
								</div>
							)}
						</div>
					</div>
							...
							{stats?.recentActivity?.length > 0 ? (
								stats.recentActivity.map((activity: any) => (
									<div key={activity.id} className="flex gap-4">
										<div className={`mt-1 shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${activity.type === 'COMMENT_ADDED' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
											{activity.type === 'COMMENT_ADDED' ? <MessageSquare size={16} /> : <FileText size={16} />}
										</div>
										<div className="flex-1">
											<p className="text-sm font-bold text-gray-900 leading-snug">{activity.title}</p>
											<p className="text-xs text-gray-500 line-clamp-1 mt-1">{activity.message}</p>
								...
								<p className="text-xs text-center text-gray-400 font-medium italic">No recent activity.</p>
							)}
						</div>
					</div>
				</div>
