...
import { LayoutList, Kanban, Loader2, RefreshCcw } from "lucide-react";
...
export default function MyTasksPage() {
    ...
    const { data: userStoriesRes, isLoading, refetch } = useQuery({
    ...
    const handleSubtaskUpdate = (subtaskId: string, status: string) => {
        updateSubtaskMutation.mutate({ subtaskId, status: status as SubtaskStatus });
    };
            ...
            <div className="flex items-end justify-between">
                ...
                <div className="flex items-center gap-2 bg-white/[0.03] border border-white/[0.05] p-1 rounded-xl">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === "kanban"
                ...
                <div className="min-h-0">
                    {viewMode === "kanban" ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {tasks.map((task: any) => (
                                <div key={task.id} className="p-6 bg-white/[0.03] border border-white/[0.05] rounded-3xl hover:border-indigo-500/30 transition-all">
                                    ...
                                    <p className="text-sm text-gray-500 line-clamp-3 mb-6">{task.description}</p>
