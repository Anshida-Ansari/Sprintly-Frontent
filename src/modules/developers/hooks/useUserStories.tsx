...
import { userStoryService } from "../services/userstory.service.tsx";
...
export const useUpdateUserStoryStatus = (projectId: string) => {
    ...
    return useMutation({
        mutationFn: ({ userStoryId, status }: { userStoryId: string; status: string }) =>
            userStoryService.updateUserStoryStatus(userStoryId, status),
        onSuccess: (res: any) => {
            queryClient.invalidateQueries({ queryKey: ["active-sprint-stories", projectId] });
