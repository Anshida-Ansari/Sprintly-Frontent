...
type ReportTab = "projects" | "sprints" | "stories" | "subtasks" | "performance";
...
export default function Reports() {
    ...
    const reportsConfig = {
        projects: {
            ...
            columns: [
                { header: "Project Name", key: "name", render: (item: any) => <span className="font-bold text-gray-900">{item.name}</span> },
                { header: "Lead", key: "leadName", render: (item: any) => (
                    <div className="flex flex-col">
                        <span className="font-medium text-gray-800">{item.leadName || "N/A"}</span>
                ...
                { header: "Timeline", key: "timeline", render: (item: any) => (
        ...
        sprints: {
            ...
            columns: [
                { header: "Sprint Name", key: "name", render: (item: any) => <span className="font-bold text-gray-900">{item.name}</span> },
                { header: "Status", key: "status", render: (item: any) => (
                    <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold uppercase tracking-wider">
                        {item.status}
                    </span>
                )},
                { header: "Stories", key: "userStoriesCount", render: (item: any) => (
                    <span className="font-black text-gray-900">{item.userStoriesCount}</span>
                )},
                { header: "Completion", key: "completionRate", render: (item: any) => (
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-black text-indigo-600">{Math.round(item.completionRate)}%</span>
                    </div>
                )},
                { header: "Duration", key: "timeline", render: (item: any) => (
        ...
        stories: {
            ...
            columns: [
                { header: "Title", key: "title", render: (item: any) => <span className="font-bold text-gray-900 max-w-xs truncate block">{item.title}</span> },
                { header: "Status", key: "status", render: (item: any) => (
                    <span className="px-2.5 py-1 bg-amber-50 text-amber-600 rounded-lg text-[10px] font-black uppercase">
                        {item.status}
                    </span>
                )},
                { header: "Points", key: "estimationPoints", render: (item: any) => (
                    <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-bold">
                        {item.estimationPoints} pts
                    </span>
                )},
                { header: "Assignees", key: "assignedUsers", render: (item: any) => (
        ...
        subtasks: {
            ...
            columns: [
                { header: "Task", key: "title", render: (item: any) => <span className="font-bold text-gray-900">{item.title}</span> },
                { header: "Assignee", key: "assignedUserName", render: (item: any) => <span className="text-sm font-medium text-gray-600">{item.assignedUserName || "Unassigned"}</span> },
                { header: "Status", key: "status", render: (item: any) => (
                    <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase ${
                        item.status === 'Done' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'
                    }`}>
                        {item.status}
                    </span>
                )},
                { header: "Hours (Est/Act)", key: "hours", render: (item: any) => (
                    <span className="text-xs font-bold text-gray-500">
                        {item.estimatedHours || 0}h / {item.actualHours || 0}h
                    </span>
                )},
                { header: "Engagement", key: "engagement", render: (item: any) => (
                    <div className="flex items-center gap-3 text-[10px] font-bold text-gray-400 capitalize">
                        <span className="flex items-center gap-1"><FileText size={12}/> {item.attachmentsCount} Attach</span>
                        <span className="flex items-center gap-1"><User size={12}/> {item.commentsCount} Comm</span>
                    </div>
                )}
            ]
        },
        performance: {
            ...
            columns: [
                ...
                { header: "Tasks", key: "tasks", render: (item: any) => (
                    <div className="text-xs font-bold">
                        <span className="text-emerald-600">{item.tasksCompleted}</span> / <span className="text-gray-400">{item.tasksAssigned}</span>
                    </div>
                )},
                { header: "Completion", key: "completionRate", render: (item: any) => (
                ...
                { header: "Time Logged", key: "totalHoursWorked", render: (item: any) => (
                    <div className="flex items-center gap-1.5 font-bold text-gray-900">
                        <Calendar size={14} className="text-indigo-400"/>
                        {item.totalHoursWorked || 0} <span className="text-gray-400 font-medium">hrs</span>
                    </div>
                )}
            ]
        }
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight">Reports Dashboard</h1>
                    <p className="text-gray-500 font-medium text-lg">Detailed operational tracking and performance logs.</p>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-6">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h2 className="font-black text-gray-900 uppercase tracking-widest text-xs">Global Filters</h2>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-gray-400 uppercase ml-1">Project</label>
                        <select 
