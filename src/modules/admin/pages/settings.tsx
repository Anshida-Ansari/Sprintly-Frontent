									...
									<div className="flex-1 min-w-0">
										<div className="flex items-start justify-between mb-3">
											<div>
												...
												<p className="text-sm text-gray-600">
													Automatically create private repositories when you
													create new projects
												</p>
											</div>
										</div>
								...
								<div className="p-6">
									<div className="flex items-center justify-between mb-6">
										<div className="flex items-center gap-3">
											...
											<div>
												...
												<h3 className="text-xl font-black text-gray-900">
													{subscription?.data?.currentPlan || "FREE"} Plan
												</h3>
											</div>
										</div>
										{subscription?.data?.currentPlan === "PRO" ? (
											...
											<span className="px-3 py-1 bg-gray-100 text-gray-600 text-xs font-black rounded-full uppercase tracking-tight">
												Free Tier
											</span>
										)}
									</div>
										...
										<div>
											<div className="flex justify-between text-sm font-bold mb-2">
												...
												<span className="text-gray-900">
													{subscription?.data?.projectCount} /{" "}
													{subscription?.data?.projectLimit === -1
														? "∞"
														: subscription?.data?.projectLimit}
												</span>
											</div>
											<div className="h-2 bg-gray-100 rounded-full overflow-hidden">
												<div
													...
													style={{
														width: `${Math.min(((subscription?.data?.projectCount || 0) / (subscription?.data?.projectLimit === -1 ? subscription?.data?.projectCount || 1 : subscription?.data?.projectLimit || 1)) * 100, 100)}%`,
													}}
												/>
											</div>
										</div>
										...
										{subscription?.data?.currentPlan === "FREE" && (
											<div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
												<p className="text-xs text-indigo-700 font-medium leading-relaxed">
													...
													for unlimited project creation and advanced team
													features.
												</p>
											</div>
										)}
						...
						<div className="flex items-start gap-4 mb-6">
							...
							<div className="flex-1">
								...
								<p className="text-gray-600 text-sm leading-relaxed">
									This will remove your GitHub connection and stop automatic
									repository creation for new projects. You can reconnect
									anytime.
								</p>
							</div>
							<button
