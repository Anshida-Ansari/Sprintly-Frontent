				...
				<div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
					<div>
						<div className="flex items-center gap-4 mb-3">
							...
							<span
								className={`px-3 py-1.5 rounded-xl text-xs font-black ${statusColors[project.status as keyof typeof statusColors] || "bg-gray-500 text-gray-600"} bg-opacity-10 text-opacity-100 uppercase tracking-widest`}
							>
								{project.status}
							</span>
						</div>
								...
								{project.members && project.members.length > 0 ? (
									<div className="space-y-4">
										{project.members.map((memberId: string, index: number) => (
											<div
												key={index}
												className="flex items-center gap-4 p-4 rounded-3xl hover:bg-gray-50 transition border border-transparent hover:border-gray-100 group"
											>
