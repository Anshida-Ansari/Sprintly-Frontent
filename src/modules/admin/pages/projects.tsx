...
export default function Projects() {
    ...
    const StatusColumn = ({ title, projects, color }: { title: string, projects: IProject[], color: string }) => (
        <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between mb-2">
                <h3 className="font-bold text-gray-700">{title}</h3>
                <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${color} bg-opacity-10 text-opacity-80`}>
                    {projects.length}
                </span>
            </div>
                ...
                <div className="flex-1 overflow-x-auto pb-8 flex flex-col">
                    ...
                    <div className="flex items-center justify-between pt-6 mt-auto border-t border-gray-100">
                        <span className="text-sm text-gray-500">
                            Page {projectData?.page} of {Math.ceil((projectData?.total || 0) / limit)}
                        </span>
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={() => {
                                    if (!isPlaceholderData && projectData?.data.length === limit) {
                                        setPage(old => old + 1)
                                    }
                                }}
                                disabled={isPlaceholderData || !projectData?.data || projectData.data.length < limit}
                                className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Next
                            </button>
