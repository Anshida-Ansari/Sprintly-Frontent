...
import { FolderOpen, Play, Clock, Trophy, ChevronRight, Plus, RefreshCw, Search, Bell, Target, Zap, ArrowUpRight } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: "Active Projects", value: "24", icon: FolderOpen, color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Running Sprints", value: "8", icon: Play, color: "text-emerald-600", bg: "bg-emerald-50" },
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Monitoring your workspace in real-time.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm">
             ...
             <div className="pr-4">
                <p className="text-[10px] font-bold text-gray-400 uppercase leading-none">Sprint 12</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:translate-y-[-4px] transition-all duration-300">
        ...
        <div className="bg-gradient-to-br from-amber-50 to-orange-100 p-6 rounded-3xl border border-orange-200 shadow-sm">
          ...
          <p className="text-xs font-bold text-orange-500 uppercase tracking-widest leading-none mb-1">Top Member</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[2rem] border border-gray-100 p-8 shadow-sm">
          ...
          <div className="space-y-6">
             ...
             {[
               { user: "Alice", action: "pushed to main", time: "2m ago", color: "bg-blue-500" },
               { user: "Bob", action: "closed Task #44", time: "15m ago", color: "bg-emerald-500" },
               { user: "Sarah", action: "created User Story", time: "1h ago", color: "bg-purple-500" },
             ].map((item, i) => (
                <div key={i} className="flex items-center gap-4 group cursor-pointer">
                   <div className={`${item.color} w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-bold shadow-sm group-hover:scale-110 transition-transform`}>
                     {item.user[0]}
                   </div>
                   <div className="flex-1">
                     <p className="text-gray-700 text-sm"><span className="font-bold text-gray-900">{item.user}</span> {item.action}</p>
                     <p className="text-[11px] font-bold text-gray-400 mt-0.5">{item.time}</p>
                   </div>
                   <ChevronRight size={18} className="text-gray-300 opacity-0 group-hover:opacity-100 transition-all mr-2" />
                </div>
             ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-indigo-100 p-8 shadow-xl shadow-indigo-100/20 relative overflow-hidden">
          ...
          <div className="space-y-3 relative z-10">
            <button className="w-full group bg-white hover:bg-indigo-600 text-gray-900 hover:text-white border border-gray-100 hover:border-indigo-600 py-4 px-6 rounded-2xl font-bold flex items-center gap-3 transition-all shadow-sm">
            ...
            <button className="w-full group bg-white hover:bg-emerald-600 text-gray-900 hover:text-white border border-gray-100 hover:border-emerald-600 py-4 px-6 rounded-2xl font-bold flex items-center gap-3 transition-all shadow-sm">
          ...
          <div className="mt-8 p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
             <p className="text-xs font-bold text-indigo-600 uppercase mb-1">Weekly Goal</p>
