				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-lg text-gray-500 font-medium">
							Drag stories into sprints to plan your next iteration.
						</p>
					</div>
			...
			{!selectedProjectId ? (
				<div className="flex flex-col items-center justify-center py-32 bg-gray-50/50 rounded-[48px] border border-dashed border-gray-200">
					...
					<p className="text-gray-400 font-bold text-center max-w-sm">
						Please select a project from the dropdown above to start planning
						your sprints.
					</p>
				</div>
			) : (
							...
							{backlog.map((story) => (
								...
								>
									<div className="flex items-start gap-3">
										<GripVertical
											size={16}
											className="text-gray-300 mt-1 opacity-0 group-hover:opacity-100 transition-opacity"
										/>
					...
					<div className="col-span-12 lg:col-span-8 space-y-6">
						{sprints.map((sprint) => (
							<div
								...
								onDragOver={allowDrop}
								className={`rounded-[40px] border-2 transition-all overflow-hidden ${
							...
							>
								...
								<div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
									<div className="flex items-start gap-4">
										<div
											className={`p-3 rounded-2xl ${sprint.status === "ACTIVE" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" : "bg-gray-100 text-gray-400"}`}
										>
											<Zap
												size={20}
												fill={
													sprint.status === "ACTIVE" ? "currentColor" : "none"
												}
											/>
										</div>
										<div>
											...
											<div className="flex items-center gap-4 text-sm font-bold text-gray-400">
												...
												<div className="flex items-center gap-1.5">
													<ScrollText size={14} />
													{
														stories.filter((s) => s.sprintId === sprint._id)
															.length
													}{" "}
													Stories
												</div>
								...
								<div className="px-6 pb-6 pt-2">
									<div className="p-4 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200 min-h-[100px] flex flex-wrap gap-3">
										{stories
											.filter((s) => s.sprintId === sprint._id)
											.map((story) => (
												<div
													key={story.id}
													draggable
										...
										{stories.filter((s) => s.sprintId === sprint._id).length ===
											0 && (
