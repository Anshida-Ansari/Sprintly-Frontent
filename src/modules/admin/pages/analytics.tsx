		...
		<div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<ChartContainer
					...
					subtitle="Distribution of tasks across the pipeline"
				>
				...
				<ChartContainer
					...
					subtitle="Tasks completed vs Total hours logged"
...
function MetricCard({ title, value, subtitle, icon, color, alert }: any) {
	...
	return (
		<div
			className={`bg-white p-6 rounded-[2.5rem] border ${alert ? "border-rose-200 animate-pulse bg-rose-50/10" : "border-gray-100"} shadow-sm transition-all hover:shadow-md group`}
		>
			<div className="flex justify-between items-start mb-4">
				<div
					className={`w-12 h-12 ${bgColors[color]} rounded-2xl flex items-center justify-center shrink-0`}
				>
					{icon}
				</div>
...
function ChartContainer({ title, subtitle, children, className }: any) {
	return (
		<div
			className={`bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm ${className}`}
		>
			<div className="mb-8">
