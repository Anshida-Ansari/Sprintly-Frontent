...
export function ProjectLevelAnalytics({ projectId }: ProjectLevelAnalyticsProps) {
    ...
    if (!analytics) return <div className="p-8 text-center text-gray-500 italic">No analytics data available for this project.</div>;
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Last 30 Days Output</p>
                        </div>
                    </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Current Task Breakdown</p>
                        </div>
                    </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm lg:col-span-2">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Tasks Completed vs Hours Logged</p>
                        </div>
                    </div>
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={userProductivity} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                <XAxis dataKey="name" stroke="#94a3b8" fontSize={11} fontWeight={800} axisLine={false} tickLine={false} />
                                <YAxis yAxisId="left" orientation="left" stroke="#6366f1" fontSize={10} fontWeight={800} axisLine={false} tickLine={false} />
                                <YAxis yAxisId="right" orientation="right" stroke="#10b981" fontSize={10} fontWeight={800} axisLine={false} tickLine={false} />
                                <Tooltip 
                                    contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 'bold' }}
                                />
                                <Bar yAxisId="left" dataKey="completedTasks" fill="#6366f1" radius={[10, 10, 0, 0]} name="Tasks Done" />
                                <Bar yAxisId="right" dataKey="totalHours" fill="#10b981" radius={[10, 10, 0, 0]} name="Hours Logged" />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm lg:col-span-2">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Completion Rate % per Sprint</p>
                        </div>
                    </div>
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={sprintAnalytics}>
                                ...
                                <Tooltip 
                                    contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 'bold' }}
                                />
                                <Bar dataKey="completionRate" fill="#8b5cf6" radius={[10, 10, 0, 0]} name="Completion %" />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
...
function StatCard({ title, value, subtitle, icon, color }: any) {
    return (
        <div className={`p-6 rounded-[2rem] bg-gradient-to-br ${color} border border-gray-100 shadow-sm flex items-start gap-4 transition-transform hover:scale-[1.02] duration-300`}>
