...
import { Search, CheckCircle2, XCircle, Eye, Filter, Building2 } from "lucide-react";
  ...
  const columns = [
    { 
      ...
      render: (company: Company) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-gray-900 leading-none">{company.companyName}</p>
    ...
    { 
      ...
      render: (company: Company) => {
        ...
        return (
          <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold border uppercase tracking-wider ${styles[status as keyof typeof styles] || "bg-gray-50 text-gray-600"}`}>
            {status}
          </span>
        );
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium">Manage and verify company registrations.</p>
        </div>
        <div className="flex items-center gap-2 text-sm font-semibold text-gray-600 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm">
          ...
          Total Companies: <span className="text-indigo-600">{data?.total || 0}</span>
        </div>
      </div>
