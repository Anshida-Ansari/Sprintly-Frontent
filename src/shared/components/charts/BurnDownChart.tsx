  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-indigo-100 p-4 rounded-2xl shadow-xl outline-none">
        <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.1em] mb-2">{label}</p>
        ...
        {payload.length > 1 && payload[1].value > payload[0].value && (
          <div className="mt-3 pt-2 border-t border-gray-100">
            <p className="text-[10px] text-rose-500 font-bold flex items-center gap-1">
              <Clock size={12} /> Delay: {(payload[1].value - payload[0].value).toFixed(1)} {unit}
            </p>
          </div>
        )}
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <YAxis 
              ...
              tickLine={false}
            />
            <Tooltip content={<CustomTooltip unit={unit} />} cursor={{ stroke: '#4f46e5', strokeWidth: 1, strokeDasharray: '5 5' }} />
            <Legend 
              verticalAlign="top" 
              align="right" 
              iconType="circle" 
